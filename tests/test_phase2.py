...
def test_write_pattern_dst_round_trips(tmp_path):
    ...
    # DST encodes a color change between layers as a single COLOR_CHANGE command.
    assert pattern.count_color_changes() == 1
    # Total stitch commands ≥ number of points (machines may add trims/jumps around color changes).
    assert pattern.count_stitches() >= sum(len(p) for _, p in layers)
